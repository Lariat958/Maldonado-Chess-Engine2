...
nn.loadModel("./../dist/models/TORCH_200EPOCHS.onnx").then(m => {
...
function flipTurn() {
  ...
  if (chess.isCheckmate()) {
    gameOverText.innerText = chess.turn() === board.getOrientation() ? "¡Perdiste!" : "¡Ganaste!";
    overlay.classList.toggle("hidden", false);
    return;
  }
  if (chess.isDraw()) {
    chess.loadPgn(chess.pgn() + " 1/2-1/2");
    gameOverText.innerText = "¡Empate!";
    overlay.classList.toggle("hidden", false);
    return;
  }
